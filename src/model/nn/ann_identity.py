...
for record in SeqIO.parse("cd-hit/results100.1", "fasta"):
    string = str(record.id)
    pdb = string[0:4]
    if pdb not in unique:
        unique.append(pdb.lower())
...
pdb_ = pdb.values
keep = []
for i in range(len(pdb_)):
    check = pdb_[i][0].lower().replace('.pdb','')

    if check in unique:
        # remove ith row in dataset
        keep.append(i)

X = X.iloc[keep]
y = y.iloc[keep]
pdb = pdb.iloc[keep].values
...
thio_un = []
mb_un = []
val_counts = val_counts.values 
red_pdb = []

for i in range(len(pdb)):
    # print(pdb[i][0])
    val = pdb[i][0].lower().replace('.pdb','')
    if val_counts[i][0] == 'metal-binding':
        red_pdb.append(val)
        if val not in mb_un:
            mb_un.append(val)
    if val_counts[i][0] == 'thioether':
        red_pdb.append(val)
        if val not in mb_un:
            thio_un.append(val)
    if val_counts[i][0].lower() == 'sulphenylation':
        red_pdb.append(val)
        if val not in mb_un:
            sulph_un.append(val)
    if val_counts[i][0].lower() == 'disulphide':
        red_pdb.append(val)
        if val not in mb_un:
            dis_un.append(val)
...
# Make Test Data
metal_X = X.iloc[100:200,:]
X = X.drop(X.index[100:200])
y_ = pd.DataFrame(y)
metal_y = y_.iloc[100:200]
y_ = y_.drop(y_.index[100:200])
y_w = y_w.drop(y_w.index[100:200])

sulph_X = X.iloc[9900:10000,:]
sulph_y = y_.iloc[9900:10000]
X = X.drop(X.index[9900:10000])
y_ = y_.drop(y_.index[9900:10000])
y_w = y_w.drop(y_w.index[9900:10000])

dis_X = X.iloc[50000:50100,:]
dis_y = y_.iloc[50000:50100]
X = X.drop(X.index[50000:50100])
y_ = y_.drop(y_.index[50000:50100])
y_w = y_w.drop(y_w.index[50000:50100])

thio_X = X.iloc[59500:59600,:]
thio_y = y_.iloc[59500:59600]
X = X.drop(X.index[59500:59600])
y_ = y_.drop(y_.index[59500:59600])
y_w = y_w.drop(y_w.index[59500:59600])
...
# callbacks
mcp_save = keras.callbacks.callbacks.ModelCheckpoint('ann_identity.h5', save_best_only=True, monitor='val_accuracy', verbose=1)
...
# Testing
model = load_model('ann_identity.h5')
