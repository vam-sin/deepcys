'''
Model training with 30% non-redundant training data
'''
# Libraries
import pandas as pd
import numpy as np
...
for record in SeqIO.parse("cd-hit/results30.1", "fasta"):
...
sulph_X = X.iloc[6000:6100,:]
sulph_y = y_.iloc[6000:6100]
X = X.drop(X.index[6000:6100])
y_ = y_.drop(y_.index[6000:6100])
y_w = y_w.drop(y_w.index[6000:6100])

dis_X = X.iloc[10000:10100,:]
dis_y = y_.iloc[10000:10100]
X = X.drop(X.index[10000:10100])
y_ = y_.drop(y_.index[10000:10100])
y_w = y_w.drop(y_w.index[10000:10100])
...
# callbacks
mcp_save = keras.callbacks.callbacks.ModelCheckpoint('ann_red.h5', save_best_only=True, monitor='val_accuracy', verbose=1)
...
# Training
weights = {0: 2.0, 1: 10.0, 2: 100.0, 3: 53.0} # d, m, s, t
